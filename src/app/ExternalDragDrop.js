...
export default function ExternalDragDrop({
  employees,
  workSpaces,
  dateRegisters,
}) {
  ...
  const handleFilterSearch = () => {
    ...
    if (filters.departmentsId) {
      filtered = filtered.filter((ev) =>
        filters.departmentsId.includes(ev.DepartmentID)
      );
    }
    if (filters.employeesId) {
      filtered = filtered.filter((ev) =>
        filters.employeesId.includes(ev.EmployeeId)
      );
    }
    // filters date
    if (filters.startTime) {
      filtered = filtered.filter(
        (ev) => new Date(ev.StartTime) > filters.startTime
      );
    }
    if (filters.endTime) {
      filtered = filtered.filter(
        (ev) => new Date(ev.EndTime) < filters.endTime
      );
    }
  ...
  const onActionBegin = (event) => {
    if (event.requestType === "eventCreate" && isTreeItemDropped) {
      ...
      const employeeName =
        employees.find((e) => e.employee_id === newEvent.EmployeeId)
          ?.employee_name || "Not Name";
      ...
      if (scheduleElement) {
        ...
        if (event.target.classList.contains("e-work-cells")) {
          const filteredData = treeviewData.filter(
            (item) =>
              item.employee_id === parseInt(event.draggedNodeData.id, 10)
          );
          ...
          <div className="schedule-container !ml-5">
            <div className="title-container">
              <h1 className="!text-3xl !font-bold">
                Schedule <span className="text-green-700">Employee </span>{" "}
              </h1>
            </div>
            <ScheduleComponent
              ref={scheduleObj}
              ...
              editorTemplate={(props) =>
                editorTemplate({ ...props, workSpaces, employees })
              }
              group={{
                  ...
                  <tr className="!row" style={{ height: "45px" }}>
                    <td className="!property-panel-content" colSpan={2}>
                      <DatePickerComponent
                        ...
                        change={(e) =>
                          setFilters((prev) => ({ ...prev, endTime: e.value }))
                        }
                      ></DatePickerComponent>
                    </td>
                  </tr>
